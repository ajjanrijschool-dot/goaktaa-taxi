...
(function () {
  ...
  /* ── Reviews ───────────────────────────────────────────────
     ...
     is worth less than no review at all. */
  (function reviews() {
    ...
    var revSend = document.getElementById('revSend');
    var revErr = document.getElementById('revError');
    var revThanks = document.getElementById('revThanks');
    var revTrap = document.getElementById('revTrap');

    function paintStars() {
      var n = Number(rating.value || 0);
      stars.querySelectorAll('.stars__b').forEach(function (b) {
    ...
    stars.querySelectorAll('.stars__b').forEach(function (b) {
      b.addEventListener('click', function () {
        rating.value = b.dataset.star;
        paintStars();
        var slot = rf.querySelector('[data-err-for="rating"]');
        if (slot) slot.textContent = '';
      });
    });

    function say(el, key) {
      var slot = rf.querySelector('[data-err-for="' + el.id + '"]');
      if (slot) slot.textContent = key ? t(key) : '';
      var wrap = el.closest('.field, .stars');
      if (wrap) wrap.classList.toggle('is-bad', !!key);
    }

    rf.addEventListener('submit', function (event) {
      event.preventDefault();
      revErr.hidden = true;
      if (revTrap.value) return;

      var bad = 0;
      if (!rating.value) { say(rating, 'reviews.err.rating'); bad++; } else say(rating, '');
      if (revName.value.trim().length < 2) { say(revName, 'reviews.err.name'); bad++; } else say(revName, '');
      if (revText.value.trim().length < 10) { say(revText, 'reviews.err.words'); bad++; } else say(revText, '');
      if (bad) return;

      if (!mailReady()) {
        console.warn('Go Aktaa: demo mode — review not sent.');
        rf.hidden = true; revThanks.hidden = false;
        return;
      }

      revSend.disabled = true;
      revSend.textContent = t('form.sending');
      fetch(MAIL.endpoint, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({
          _subject: 'Review — ' + rating.value + '/5 from ' + revName.value.trim(),
          _gotcha: revTrap.value,
        ...
        revSend.disabled = false;
        revSend.textContent = t('reviews.send');
        if (res.ok) { rf.hidden = true; revThanks.hidden = false; return; }
        revErr.innerHTML = t('form.failed');
        revErr.hidden = false;
      }, function () {
        revSend.disabled = false;
        revSend.textContent = t('reviews.send');
